...
import (
  ...
  _ "github.com/lib/pq"
  "github.com/ausaafnabi/urlshortner/models"
  base62 "github.com/ausaafnabi/urlshortner/utils"
)
...
func (driver *DBClient) GetOriginalURL(w http.ResponseWriter, r *http.Request) {
  ...
  func (driver *DBClient) GenerateShortURL(w http.ResponseWriter,e *http.Request) {
    var id int
    var record Record
  ...
  func main(){
    ...
    r:= mux.NewRouter()
    r.HandleFunc("v1/short/{encoded_string:[a-zA-Z0-9]*}",db.client.GetOriginalURL).Method("GET")
    r.HandleFunc("v1/short",dbclient.GenerateShortURL).Methods("POST")
    srv := &http.Server {
      Handler: r,
      AddrL "127.0.0.1:8000",
      WriteTimeout: 15 *time.Second.
      ReadTimeout: 15 *time.Second,
    }
    log.Fatal(srv.ListenAndServ())
  }
}
